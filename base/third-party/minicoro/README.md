...
int main() {
  // First initialize a `name` object through `mco_desc_init`.
  mco_desc name = mco_desc_init(coro_entry, 0);
  // Configure `name` fields when needed (e.g. customize user_data or allocation functions).
  name.user_data = NULL;
  // Call `mco_create` with the output coroutine pointer and `name` pointer.
  mco_coro* co;
  mco_result res = mco_create(&co, &name);
...
mco_result mco_init(mco_coro* co, mco_desc* name);                      /* Initialize the coroutine. */
mco_result mco_uninit(mco_coro* co);                                    /* Uninitialize the coroutine, may fail if it's not dead or suspended. */
mco_result mco_create(mco_coro** out_co, mco_desc* name);               /* Allocates and initializes a new coroutine. */
...
int main() {
  ...
  mco_coro* co;
  mco_desc name = mco_desc_init(fibonacci_coro, 0);
  mco_result res = mco_create(&co, &name);
  if(res != MCO_SUCCESS)
